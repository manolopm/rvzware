...
//
       << "UTMLatMin:               " << src.theUTMLatMin << std::endl
       << "UTMLatMax:               " << src.theUTMLatMax << std::endl
       << "UTMLonMin:               " << src.theUTMLonMin << std::endl
       << "UTMLonMax:               " << src.theUTMLonMax << std::endl
//
...
//PJ
   if(src.SRS().size() > 0)
   {
      out << std::endl;
      out << "SRS: ";
      for(idx = 0; idx < src.SRS().size()-1; ++idx)
      {
         out << src.getSRS(idx) << ", ";
      }
      out << src.SRS()[src.SRS().size()-1] << " ";
   }
//end PJ
...
wcsCapabilitiesState::~wcsCapabilitiesState()
{
}
...
void wcsCapabilitiesState::inherit(const wcsCapabilitiesState& src)
{
...
//
   theUTMLatMin = src.theUTMLatMin;
   theUTMLatMax = src.theUTMLatMax;
   theUTMLonMin = src.theUTMLonMin;
   theUTMLonMax = src.theUTMLonMax;
//
...
wcsRefPtr<wcsCapabilitiesState> wcsCapabilitiesState::child(unsigned int idx)
{
...
const wcsRefPtr<wcsCapabilitiesState> wcsCapabilitiesState::child(unsigned int idx)const
{
   if(idx < theChildren.size())
   {
      return theChildren[idx];
...
void wcsCapabilitiesState::addChild(wcsCapabilitiesState* child)
{
   theChildren.push_back(child);
}


bool wcsCapabilitiesState::getLatLonBoundingBox(double& minLat,
                                                double& minLon,
                                                double& maxLat,
                                                double& maxLon)const
{
...
bool wcsCapabilitiesState::getUTMLatLonBoundingBox(double& minLat,
                                                double& minLon,
                                                double& maxLat,
                                                double& maxLon)const
{
   ...
   if(hasChildren())
   {
      ...
      for(idx = 0; idx < theChildren.size(); ++idx)
      {
         ...
         if(theChildren[idx].valid()&&
            theChildren[idx]->getUTMLatLonBoundingBox(tempMinLat,
                                                   tempMinLon,
                                                   tempMaxLat,
                                                   tempMaxLon))
         {
   ...
   tempMinLat = atof(theUTMLatMin.c_str());
   tempMinLon = atof(theUTMLonMin.c_str());
   tempMaxLat = atof(theUTMLatMax.c_str());
   tempMaxLon = atof(theUTMLonMax.c_str());
