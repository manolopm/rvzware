...
wcsMyTime operator+(const wcsMyTime & a, const wcsMyTime & b)
{
	...
	while (result.day > (days[(result.month-1)%12] + wcs_leap_days(result.year, result.month)))
	{
		result.day -= days[(result.month-1)%12] + wcs_leap_days(result.year, result.month);
		result.month++;
	}
...
wcsMyTime operator-(const wcsMyTime & a_src, const wcsMyTime & b_src)
{
	if (a_src < b_src)
		return b_src - a_src;
	...
	{
		a.month--;
		a.day += days[(a.month+11)%12] + wcs_leap_days(a.year, a.month);
	}
...
void wcs_parse_time(wcsMyTime & t, const std::string & str)
{
	...
	// year
	loc = (int) str.find_first_of("0123456789");
	loc2 = (int) str.find_first_not_of("0123456789", loc);
	if (loc < 0 || loc == loc2)
		return;
	...
	// month
	//loc = strcspn(cp, "- \t");
	loc = (int) str.find_first_of("0123456789", loc2);
	loc2 = (int) str.find_first_not_of("0123456789", loc);
	if (loc < 0 || loc == loc2)
		return;
	...
	if (loc < 0 || loc == loc2)
		return;
	t.sec = (float) atof(str.substr(loc, loc2-loc).c_str()); 
}
...
void wcs_parse_period(wcsMyTime & t, const std::string & str)
{
	...
	while (cp && *cp)
	{
		int loc = (int) strspn(cp, "0123456789.");
...
{
	wcsMyTime t = { -1, -1, -1, -1, -1, -1 };
	wcs_parse_time(t, str);
	return wcs_iso8601_time(t);
}
