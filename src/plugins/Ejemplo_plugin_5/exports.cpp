...
class GraphicE3DCallBack : public Model3DCallBack
{
	public:
		...
		virtual void Animate(Callable* callable, cpwTime &time_stamp){
			...
			// calculamos la duración entre el instante actual y el inicial
...
{	
	...
	// miramos si se pulsó el botón OK
	if (ui->GetReturnCode() == wxID_OK) {
		...
		// iteramos tantos segundos según indicado por el usuario
		for (int n=1; n<ui->GetSeconds(); n++) {

			// calculamos el gradiente en la posición actual
			double grad[2];
		...
		// parámetros para el panel de animación
		cpwTime tim;
		tim.seconds = 0;
		tim.miliseconds = 0;
		el->SetStartTime(tim);
