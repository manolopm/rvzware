...
{
	SetTitle(wxT(application_title));
	SetIcon(wxNullIcon);
	...
	wxHyperlinkCtrl *link = new wxHyperlinkCtrl(this, wxID_ANY, _T(application_url), _T(application_url), wxPoint(TIMELINE_CORNERX+TIMELINE_WIDTH+40,TIMELINE_CORNERY+50+10+20));
	if (!(ApplicationConfiguration::GetInstance()->IsThemed()))
	{
		wxColour c_pen   = cpw::ApplicationConfiguration::GetInstance()->GetPageColour();
...
void UIHelp::render(wxDC& dc)
{
	...
	if (!(ApplicationConfiguration::GetInstance()->IsThemed()))
	{
		...
		wxColour c_page = ApplicationConfiguration::GetInstance()->GetPageColour();
	...
	dc.DrawRotatedText(_T(application_version), TIMELINE_CORNERX+TIMELINE_WIDTH+40,TIMELINE_CORNERY+45+15, 0);
	dc.SetFont(wxFont(12,wxDEFAULT,wxFONTSTYLE_NORMAL,wxBOLD,true));
	dc.DrawRotatedText(_T(application_name),TIMELINE_CORNERX+TIMELINE_WIDTH+40,TIMELINE_CORNERY+30, 0);
}
